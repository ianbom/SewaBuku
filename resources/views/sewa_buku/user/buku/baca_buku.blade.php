                                ...
                                <div id="bab-{{ $detail->id_detail_buku }}" class="mb-12">
                                    <div class="flex items-center gap-4 mb-6">
                                        ...
                                        @if ($detail->id_detail_buku == $babTerakhirDibaca)
                                            <span class="px-3 py-1 bg-green-100 text-green-600 text-sm rounded-full">Last
                                                Read</span>
                                        @endif
                                    </div>
                        ...
                        @if ($checkLangganan)
                            <div class="flex justify-between items-center mt-12 pt-6 border-t">
                                <form
                                    ...
                                    method="POST" class="mt-2 w-full flex justify-between gap-4">
                                    @csrf

                                    @if (in_array($buku->id_buku, $diselesaikan))
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600 w-full flex items-center justify-center gap-2">
                                            <i class="fas fa-trash-alt"></i> Cancel Marked as Finished
                                        </button>
                                    @else
                                        <button type="submit"
                                            class="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-600 w-full flex items-center justify-center gap-2">
