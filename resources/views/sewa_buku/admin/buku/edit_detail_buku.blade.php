            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        ...
                        <p class="text-subtitle text-muted">Perbarui atau tambahkan detail untuk buku
                            "{{ $buku->judul_buku }}"</p>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="{{ route('admin.updateBuku.edit', $buku->id_buku) }}" method="POST"
                                    enctype="multipart/form-data" id="detailBukuForm">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    @foreach ($detailWithQuiz as $key => $detail)
                                        <div class="border p-4 mb-4 rounded detail-buku">
                                            ...
                                            <div class="mb-3">
                                                <label for="detail_buku[{{ $key }}][bab]"
                                                    class="form-label">Bab</label>
                                                <input type="text" name="detail_buku[{{ $key }}][bab]"
                                                    class="form-control"
                                                    value="{{ old("detail_buku.$key.bab", $detail->bab) }}" required>
                                            </div>

                                            <div class="mb-3">
                                                <label for="detail_buku[{{ $key }}][isi]"
                                            ...
                                            <div class="mb-3">
                                                <label for="detail_buku[{{ $key }}][audio]"
                                                    class="form-label">Audio</label>
                                                <input type="file" name="detail_buku[{{ $key }}][audio]"
                                                    class="form-control" accept="audio/mp3">
                                                @if ($detail->audio)
                                                    <div class="form-check mt-2">
                                                        <input type="checkbox"
                                                            name="detail_buku[{{ $key }}][keep_existing_audio]"
                                                            value="1" class="form-check-input" checked>
                                                        <label class="form-check-label">Pertahankan audio yang ada</label>
                                                        <p class="text-muted small">Audio saat ini: {{ $detail->audio }}
                                                        </p>
                                                        <input type="hidden"
                                                            name="detail_buku[{{ $key }}][existing_audio]"
                                                            value="{{ $detail->audio }}">
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="detail_buku[{{ $key }}][is_free_detail]"
                                                    id="is_free_detail" class="form-select">
                                                    <option value="0"
                                                        {{ old("detail_buku.$key.is_free_detail", $detail->is_free_detail) == 0 ? 'selected' : '' }}>
                                                        Tidak</option>
                                                    <option value="1"
                                                        {{ old("detail_buku.$key.is_free_detail", $detail->is_free_detail) == 1 ? 'selected' : '' }}>
                                                        Ya</option>
                                                </select>
                                            </div>
...
{{-- 
                                            ...
                                            @if ($detail && $detail->bab && $detail->isi)
                                                <div class="mb-3">
                                                    @if ($detailWithQuiz->contains($detail))
                                                        <a href="{{ route('quiz.show', $detail->id_detail_buku) }}"
                                                            class="btn btn-success btn-sm">Lihat Quiz</a>
                                                    @else
                                                        <a href="{{ route('quiz.create', $detail->id_detail_buku) }}"
                                                            class="btn btn-primary btn-sm">Buat Quiz</a>
                                                    @endif
                                    ...
                                    <button type="button" id="addDetailButton" class="btn btn-success mb-3">Tambah
                                        Detail Baru</button>
