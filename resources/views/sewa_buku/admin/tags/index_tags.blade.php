                                        ...
                                        @foreach ($tags as $tag)
                                            <tr>
                                                ...
                                                <td class="text-center justify-center items-center">
                                                    <a href="{{ route('admin.tags.edit', $tag->id_tags) }}"
                                                        class="btn btn-warning btn-sm">Edit</a>
                                                    <form action="{{ route('admin.tags.delete', $tag->id_tags) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger btn-sm"
                                                            onclick="return confirm('Yakin ingin menghapus tags ini?')">
                                                            Hapus
                                                        </button>
                                                    </form>
                                                </td>
                            ...
                            <form action="{{ route('admin.tags.store') }}" method="POST">
                                ...
                                <div class="row">
                                        ...
                                        <select name="id_child" class="form-control">
                                            ...
                                            @foreach ($parent as $tag)
                                                <option value="{{ $tag->id_tags }}"
                                                    {{ old('id_child') == $tag->id_tags ? 'selected' : '' }}>
                                                    {{ $tag->nama_tags }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" name="nama_tags" value="{{ old('nama_tags') }}"
                                            class="form-control" placeholder="Masukkan Nama Tags">
                                        @error('nama_tags')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
